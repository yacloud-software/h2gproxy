...
func (p *Probe) NoAuthJSONGet() *Result {
	res := &Result{
		TestName: "noauth-html-get",
		URL:      fmt.Sprintf("%s/%s", p.BaseURL(), "noauth/json"),
	}
...
func (p *Probe) NoAuthJSONPost() *Result {
	res := &Result{
...
func (p *Probe) AuthJSONGet() *Result {
	res := &Result{
		TestName: "auth-html-get",
		URL:      fmt.Sprintf("%s/%s", p.BaseURL(), "auth/json"),
	}
	clid := utils.RandomString(256)
	v := url.Values{"echo": []string{clid}}
	h := res.HTTP()
...
func (p *Probe) AuthJSONPost() *Result {
	res := &Result{
...
func (p *Probe) FormAuthJSONGet() *Result {
	res := &Result{
